...
- While editing code, run only the affected tests; run the full `scripts/test.sh all` once before
  opening a pull request and paste its summary into the PR body. CI repeats everything (lint, JS,
  package build, and pytest against a NetBox matrix), but locally you get the answer sooner.
