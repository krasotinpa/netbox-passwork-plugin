...
SecretDetailView.get()
    ...
    │     if not numeric), then bound_object_access() — the object must exist and be viewable
    │     under the user's ObjectPermission constraints (ADR-0002) → 404 object_not_found,
    │     then PassworkBinding.objects.filter(object_type, object_id,
    │     passwork_item_id).exists() → 404 binding_not_found (guards against an arbitrary pw_id);
    │     all failures go through ApiError
