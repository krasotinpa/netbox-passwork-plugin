...
PassworkView.dispatch() → PassworkGateway (netbox_passwork/gateway.py)
   │  six operations (login/confirm_totp/get_item/list_vaults/search_items/require_session);
...
`pwOpenPicker`, `pwLoadPickerFolders`, `pwPickerSearch`, `pwRevealSecret`, `pwCopyField`,
